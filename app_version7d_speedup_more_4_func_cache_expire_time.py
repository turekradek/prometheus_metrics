...
# Define a dictionary to store the cached results
cached_results = {}
@lru_cache(maxsize=128)
def get_cached_results(task_id):
    """
    Helper function to retrieve the cached results for a given task_id.
    """
    if task_id in cached_results:
        timestamp, results = cached_results[task_id]
        if dt.now(timezone.utc) - timestamp < CACHE_EXPIRATION_TIME:
            return results
    return None

def set_cached_results(task_id, results):
    """
    Helper function to cache the results for a given task_id.
    """
    cached_results[task_id] = (dt.now(timezone.utc), results)
...
def ingest():
    ...
    tasks_ids = [
        'ingest', 'signal_extractor','signal_synchronizer', 'endurance_run',
    ]
    for task_id in tasks_ids:
        airflow_restapi_dags(task_id)
    
    # task_id='signal_extractor'
    # show_df =airflow_restapi_dags(task_id)
    show_df = ','.join(tasks_ids)
    now = dt.now().strftime("%d-%m-%Y %H:%M:%S")
    return f"""<meta http-equiv="refresh" content="90">
            {now} <br>
            {dt.now() - start}<br>
...
def signal_synchronizer():
    ...
    # task_id='endurance_run'
    tasks_ids = [
        'signal_synchronizer', 'endurance_run',
    ]
    for task_id in tasks_ids:
        airflow_restapi_dags(task_id)
...
def data_catalog():
    ...
    tasks_ids = [
        'data_catalog', 'data_catalog',   'video_preview', 'marker_import_tablet_label', 'auto_labelling',
        ]
    for task_id in tasks_ids:
        airflow_restapi_dags(task_id)
...
def video_preview():
    ...
    tasks_ids = [ 
        'video_preview', 'video_preview', 'image_extractor', 'video_creator', 'dq_repro_rules',
        ]
    for task_id in tasks_ids:
        airflow_restapi_dags(task_id)
